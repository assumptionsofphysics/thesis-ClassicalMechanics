...
\end{itemize}
	...
	We have seen that the volume element of our manifold $d\xi^{2n}$ is conserved under a canonical transformation. How can we relate this differential to our measure $dS$? What units will we use to count states? The convention in statistical mechanics is to use $\hbar$. We will write $p = \hbar k$ so that $p$ depends not only on the unit of $q$, but also on the units used for the states. We then get the relation $dS = dq^n \land dp_n = \hbar^n d\xi^{2n}$.
