...
\tikzset{shift={(current page.center)},xshift=3cm}
   \begin{axis}[view={140}{30},
	...
	\addplot3+[line width=.5pt,no markers,samples=70,color=green, samples y=0,domain=0:5*pi,variable=\t]
                                      (-{exp(-.5*\t)*cos(\t r)},.5*\t,-{exp(-.5*\t)*sin(\t r)});
	\addplot3+[line width=.5pt,no markers,samples=70,color=cyan, samples y=0,domain=0:5*pi,variable=\t]
                                      ({-exp(-.5*\t)*sin(\t r)},.5*\t,{exp(-.5*\t)*cos(\t r)});
\addplot3+[line width=.5pt,no markers,samples=70,color=teal, samples y=0,domain=0:5*pi,variable=\t]
                                      ({exp(-.5*\t)*sin(\t r)},.5*\t,-{exp(-.5*\t)*cos(\t r)});
	\addplot3+[line width=.5pt,no markers,samples=70,color=blue,  samples y=0,domain=0:5*pi,variable=\t]
...
\addplot3+[line width=.5pt,no markers,samples=70,color=teal, samples y=0,domain=0:5*pi,variable=\t]
  ...
  \end{axis}
\end{tikzpicture}
\caption{The evolution of a damped harmonic oscillator in phase space. Evidently this vector field is not divergenceless. At some point }
\end{figure}
