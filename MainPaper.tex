...
\end{assump}
	...
	A system undergoes deterministic and reversible evolution if there is a bijection from past states to future states. This is a necessary but not sufficient condition for our definition of this kind of evolution. We will see that not all systems for which we can write this bijection will undergo Hamiltonian evolution.
	
	Imagine we have a one dimensional damped harmonic oscillator. We have a one to one/injective (and onto/surjective) map from past states to future states. But we also know that as the system evolves the density of these states will concentrate and asymptotically approach the rest state. Our state variables will be position and momentum. Our state space will be a 2D plane. If we imagine that we copy this state space over time we can map the evolution of each classical particle. The evolution of each particle can be written as $\xi^a(t)$. If we change infinitesimally in time we can write $\xi^a(t + dt) = \xi^a + \frac{d\xi^a}{dt}dt$. We know that $\xi^a(t)$ is differentiable because $\rho$ is always defined. \emph{???} We then will write $S^a = \frac{d\xi^a}{dt}$. This will be a vector field in $\mathbb{R}^3$. 
