...
\end{defn}
	...
	The Jacobian of a change of variables will give the conversion factor of the units of the density. But we also know that the units of $\mu(U)$ will not change as the fraction of a system occupying a region of state space will always be a pure number. Earlier we said the Jacobian of a canonical transformation is \textit{unitary}. This means that not only is the value equal to one, but also that it is unit-less. We also know that a change of unit variables will induce a change in the other state variables because every state variable is either a unit variable, or its units are derived from the definition of a unit variable. So we will ourselves change only the unit variables; the other state variables must change in accordance with the new definitions of units such that the units of density do not change.
...
\end{prop}
	...
	What are the units of the conjugate variables $k_i$? Taking an equality we used earlier we have $$\left|\frac{\partial \hat{k}_j}{\partial k_i}\right| = \left|\frac{d q^i}{d \hat{q}^j}\right|.$$ Evidently the change of units in $q^i$ induces an inverse change in the units of $k_i$. 
...
\end{itemize}
	...
	We have seen that the volume element of our manifold $d \xi^{2n}$ is conserved under a canonical transformation. How can we relate this differential to our measure $dS$? What units will we use to count states? The convention in statistical and quantum mechanics is to use $\hbar$ as the units of number of states. We will write $p = \hbar k$ so that $p$ depends not only on the unit of $q$, but also on the units used for the count of states. We then get the relation $dS = dq^n \land dp_n = \hbar^n d\xi^{2n}$. Each individual pair of $\{q^i,p_i\}$  variables will form a 2D surface in phase space. These surfaces will be orthogonal to one another; we know this from the fact that we can change unit variables independently of one another. We will call these pairings \textit{degrees of freedom}.

\begin{defn}
	An individual degree of freedom (d.o.f.) is a pair $\{q^i,p_i \}$ such that $dS = {\displaystyle \prod_{i=1}^{n}} dq^i \land dp_i = dq^n \land dp_n = \hbar^n d\xi^{2n}$. Degrees of freedom are orthogonal 2D surfaces in phase space.
\end{defn}

	Because of the orthogonality of our d.o.f.'s we can break our count of states into a count over each individual surface. For example imagine we have a phase space of four dimensions where $q^1,q^2$ are positions along the $x$ and $y$ axes and $p_1,p_2$ are momenta along those axes. We can count the total number of possible states by first counting the possible configurations of position and momentum in each direction, then multiplying these two counts together. In our space the degrees of freedom are completely separate meaning that the choice of a configuration in one has no effect on the choice of a configuration in the other.

\begin{prop}
	For each independent degree of freedom, the area given $\int_U \hbar dq^i dk_i = \int_U dq^i dp_i$ quantifies the number of possible configurations within the region $U$.The total number of possible states in a volume is given by the product ${\displaystyle \prod_{i=1}^{n}} (\int_{U_i} dq^i dp_i)$ where $U_i$ is the 2D projection of the volume onto each degree of freedom $\{q^i,p_i\}$.
\end{prop}
...
\end{assump}
	...
	A system undergoes deterministic and reversible evolution if there is a bijection from past states to future states. This is a necessary but not sufficient condition for our definition of this kind of evolution. We will see that not all systems for which we can write this bijection will undergo Hamiltonian evolution. As in section 2.2 we will begin in the simpler two-dimensional case. This will illustrate the necessary and sufficient conditions for a system to undergo deterministic and reversible evolution before we move to the general $2n$ dimensional general case. 
	
	Imagine we have a one dimensional damped harmonic oscillator. We have a one to one/injective (and onto/surjective) map from past states to future states. But we also know that no matter the initial state, as the system evolves it will asymptotically approach its rest state. Because our state variables will be position and momentum we know that as the system evolves all of the classical particles will approach the same state. 
	
	We know that in the real world all measurements have finite precision. So at some point in the above example we will not be able to tell the state with the oscillator barely moving and the rest state; they will look the same. Thinking of this in terms of our knowledge of the system we see that as time progress and our states collapse towards the rest state, we lose information about the system. To avoid this problem we will say that for a system to undergo deterministic and reversible evolution, it must not only have a bijection from past states to future states, but also the density of states $\rho$ must be invariant in time. Now this does not mean that the classical particles cannot change state. It means that if we have a certain number of classical particles in a initial state, all of those particles and no others will end up in the same final state.
	
	If we transport our 2D state space in time $t$, we can map the evolution of each classical particle in $\mathbb{R}^3$. The evolution of each particle can be written as $\xi^a(t)$. If we change infinitesimally in time we can write $\xi^a(t + dt) = \xi^a + \frac{d\xi^a}{dt}dt$. We know that $\xi^a(t)$ is differentiable because $\rho$ is always defined. \emph{???} We then will write $S^a = \frac{d\xi^a}{dt}$. This vector field will  
	
	Diagram
	
	In our damped oscillator example this vector field will have a nonzero divergence i.e. 
