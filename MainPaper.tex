...
\end{prop}
	...
	We know we can write our $\mu(U)$ in terms of our state variables as $\mu(U) = \int_{\xi(U)} \rho(\xi^a)d\xi^a$. If we change our state variables we know that that fraction of the system in a region $U \subset \mathcal{S}$ will not change. Let $\hat{\xi}^b = \hat{\xi}^b(\xi^a)$ be our change of variables. This gives us $\mu(U) = \int_{\xi(U)} \rho(\xi^a)d\xi^a = \int_{\hat{\xi}(U)} \rho(\hat{\xi}^b)d\hat{\xi}^b$. Let's think about this in more concrete terms. We know to find the total mass in a region from a density we write the integral $$\int dm = \int \rho dV.$$ If we say we want to conserve the value of this integral but change the value of $\rho$ we know the value of $dV$ must change accordingly. Our constraint however, tells us that the value of $\rho$ cannot change, thus we know the value of our "volume" element $d\xi^a$ cannot change either.
	
	Using our transformation rule we can write $\int_{\hat{\xi}(U)} \rho(\hat{\xi}^b)d\hat{\xi}^b = \int_{\xi(U)} \rho(\hat{\xi}^b)\left|\frac{\partial\hat{\xi}^b}{\partial\xi^a}\right|d\xi^a = \int_{\xi(U)} \rho(\xi^a)d\xi^a$. We already know $\rho(\xi^a) = \rho(\hat{\xi}^b)$ by proposition [placeholder]; we can drop the integral to get $\rho(\xi^a) = \rho(\hat{\xi}^b)\left|\frac{\partial\hat{\xi}^b}{\partial\xi^a}\right|$ which implies that $\left|\frac{\partial\hat{\xi}^b}{\partial\xi^a}\right| = 1$. Evidently this means that the value of the volume element\footnote{this volume element describes volume on our manifold $\mathcal{S}$.} doesn't change as expected. It is important to note here that $\mu$ is always unit-less. It is always the fraction of the system occupying a region of space and is thus a pure number. This will be important later.
...
\renewcommand{\arraystretch}{1.3}
 ...
 \hline
 Space-Time & $\mathcal{M}$ & Physical space. In our case three-dimensional without temporal axis. \\ [1ex] 
 \hline
\end{tabular}
\end{center}
...
\renewcommand{\arraystretch}{2.5}
 ...
 \hline\hline
 Distribution or Density of States & $\rho(s)$ or $\rho(\xi^a)$ & $\mathcal{S} \to \mathbb{R}$ \\
 \hline
 Counting function & $\mu(U)$ & $\mathbb{P}(\mathcal{S}) \to \begin{cases}
...
\emph{Talk about unit system in general: some choices are free, some are dependent. Give example of dependent/independent unit variables (like velocity down). Dimensionless 1 math throws away units}
	...
	We know that to define a density we need some kind of units that relate areas of the manifold to the fraction of the system found in those areas. Returning to our mass example we saw that we needed to define units of distance and of mass in order to describe this density. In the case of $\rho$ the unit of "mass" is $\mu(U)$, the fraction of the system found within a region $U$. In order to understand our density we also need to define our unit of "distance" over our state space. Now not that our state variables can indicate any number of quantities; they do not necessarily label distance. They could label something like temperature or electric charge, but they define a notion of "distance" over the state space for the purpose of defining volumes. 
	
	We must remember that we are working with physical systems here. This means that the units of our state variables are important. Some of our state variables will define a unit and others will have their units derived from the units of other state variables. For example if distance in a direction is defined in terms of meters, velocity in that direction must be defined in meters per unit time. If we use seconds as our unit time for velocity then acceleration must be meters per second squared. The units of velocity and acceleration are derived from the definition of the units of distance. Changing the definition of a fundamental unit will also change the definitions of all the units derived from it. 
...
\end{itemize}
	...
	We have seen that the volume element of our manifold $d\xi^a$ is conserved under a canonical transformation. How can we relate this differential to our measure $dS$? What units will we use to count states? The convention in statistical mechanics is to use $\hbar$. We will write $p = \hbar k$ so that $p$ depends not only on the unit of $q$, but also on the units used for the states. We then get the relation $dS = dq^i \land dp_i = \hbar d\xi^a$.

\begin{defn}
	A degree of freedom is a pair $\{q^i,p_i\}$ such that $dS = dq^i \land dp_i$. Degrees of freedom are orthogonal 2D surfaces in phase space.
\end{defn}
...
\end{prop}
	...
	How do we determine if a pair of variables forms a degree of freedom? We will define an operator that returns either 0 if the variables do not form a d.o.f. or returns the unit change of the area $\frac{dfdg}{dS}$.
