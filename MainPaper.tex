...
\end{defn}
	...
	The Jacobian of a change of variables will give the conversion factor of the units of the density. But we also know that the units of $\mu(U)$ will not change as the fraction of a system occupying a region of state space will always be a pure number. Earlier we said the Jacobian of a canonical transformation is \textit{unitary}. This means that not only is the value equal to one, but also that it is unit-less. 
...
\begin{prop}
	The units of conjugate variables $\{k_i\}$ are the inverse of the units of the corresponding unit variables $\{q^i\}$. The volume element $d\xi^a = dq^i \land dk_i$ is invariant in value and units under a canonical change of variables.
\end{prop}
