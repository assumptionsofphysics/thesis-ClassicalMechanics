...
\end{assump}
	...
	Remember, this conservation does not mean that the system cannot change states; it means only that if a certain number of classical particles begin in one state, that exact number of particles will end together in the same state. For example, if half of the system begins in state A and one of these particles is later found in state B then we know that exactly half of the system is found in state B at that time. 
