...
\end{defn}
	...
	If our state space is continuous, we must make two changes. $\rho : \mathcal{S} \to \mathbb{R}$ becomes a density and instead of a sum we use an integral: $\mu(U) = \int_{U} \rho(s) d\mathcal{S}$ where $d\mathcal{S}$ gives the number of states in an infinitesimal area.
