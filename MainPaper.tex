...
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth]{images/diagram2.jpg}
	\caption{A description}
	\label{fig_functionmap}
\end{figure}
...
% We use properties and quantities to identify states. State variables are continuous quantities that we use to identify states.	Convince the reader that a state variable is a map from a state to a continuous value.
...
% Now that we have state variables, we can express the distribution in terms of the state variables. Show that there is a problem in doing that.

% Show what are the requirements that the density has to satisfy to address the problem.
